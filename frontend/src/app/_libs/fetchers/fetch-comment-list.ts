import { CommentListResponse, ErrorResponse } from "../../_types/responses/comment-list-response";

export async function fetchCommentList(contentId: string) {
  const message = '지금은 댓글 목록을 불러올 수 없습니다. 잠시 후 다시 시도해주세요.';
  ...
  let responseJson: CommentListResponse;
  try {
  ...
  return { commentListResponse: responseJson };
}




// import { CommentListResponse, ErrorResponse } from "../_types/comment-list-response";

// export async function fetchCommentList(url: string) {
//   const message = '지금은 댓글 목록을 불러올 수 없습니다. 잠시 후 다시 시도해주세요.';
